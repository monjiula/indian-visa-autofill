...
async function loadProfiles(mode, showHidden = false) {
  ...
  Object.keys(storage).forEach((key) => {
    if (['lastServiceMode', 'lastSelectedProfile', 'ga_client_id', 'geminiApiKey'].includes(key)) return;
  ...
  filtered.forEach(p => {
    ...
    if (renameBtn) {
      renameBtn.onclick = async () => {
        ...
        if (newName && newName !== p.data._savedName) {
          ...
          await chrome.storage.local.set({ [newKey]: updated });
          await chrome.storage.local.remove(p.key);
          if (p.selected) await chrome.storage.local.set({ lastSelectedProfile: newKey });
          loadProfiles(currentMode, false);
        }
      };
    }
    ...
    if (hideBtn) {
      hideBtn.onclick = async () => {
        if (confirm('Hide this profile?')) {
          p.data._isHidden = true;
          await chrome.storage.local.set({ [p.key]: p.data });
          loadProfiles(currentMode, false);
        }
      };
    }
...
async function init() {
  const { lastServiceMode } = await chrome.storage.local.get(['lastServiceMode']);
  switchMode(lastServiceMode || 'BGD');
  
  await loadApiKey();
  loadTempIds(); // Load Temporary IDs
  
  // Set default model
  const storedModel = await chrome.storage.local.get(['geminiModel', 'iv_autoDeleteDays', 'iv_promptBGD', 'iv_promptPassport']);
  if(storedModel.geminiModel) $('modelSelect').value = storedModel.geminiModel;
  if(storedModel.iv_autoDeleteDays) $('autoDeleteSelect').value = storedModel.iv_autoDeleteDays;
  
  if(storedModel.iv_promptBGD) $('promptBGD').value = storedModel.iv_promptBGD;
  if(storedModel.iv_promptPassport) $('promptPassport').value = storedModel.iv_promptPassport;
