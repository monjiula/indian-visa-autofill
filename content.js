...
async function injectFloatingWidget() {
    ...
    widget.innerHTML = `
        <style>
            ...
            #ivap-fab {
                ...
                transition: all 0.3s; animation: ivapSlideIn 0.3s ease;
            }
            ...
            #ivap-widget.ivap-dark-mode .ivap-docs { border-top-color: rgba(255,255,255,0.1); }
            #ivap-widget.ivap-dark-mode .ivap-doc-btn { background: rgba(30, 30, 46, 0.9); border-color: #475569; color: #cbd5e1; }
            #ivap-widget.ivap-dark-mode .ivap-doc-btn:hover { border-color: #10b981; color: #10b981; background: rgba(16,185,129,0.1); }
        </style>
    ...
    // FAB (floating action button when minimized)
    ...
    fab.onclick = () => {
        ...
        void widget.offsetHeight; // trigger reflow
    ...
    // Profile change saves selection
    document.getElementById('ivap-profile-select').onchange = async (e) => {
        if (e.target.value) {
            try {
                ...
                console.warn("IV Autofill: Extension context invalidated. Please refresh the page.", err);
            }
        }
    };
    ...
    // Disable slide-in animation if navigating between form pages to feel more seamless
